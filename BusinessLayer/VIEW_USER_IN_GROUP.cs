...
namespace BusinessLayer
{
    public class VIEW_USER_IN_GROUP
    {
        ...
        public List<tb_SYS_USER>getGroupByUser(string macty, string madvi, int idUser)
        {
            ...
            List<V_USER_INGROUP> lst = db.V_USER_INGROUP.Where(x=>x.MEMBER == idUser && x.MACTY==macty && x.MADVI==madvi).ToList();
            tb_SYS_USER u;
		...
		public List<V_USER_INGROUP> getUserInGroup(string madvi, string macty, int idGroup)
        {
            return db.V_USER_INGROUP.Where(x => x.MADVI == madvi &&x.GROUP==idGroup && x.MACTY == macty && x.ISGROUP == false && x.DISABLED == false).ToList();
        }
    }
}
